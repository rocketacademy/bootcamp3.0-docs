...
2. Understand how React loads through the DOM on a webpage
3. Understand how to install packages in an NPM project
4. Understand how to encapsulate UI elements in components and pass parameters to them with props
5. Understand how to store state in components and use that state to manipulate UI

## Introduction

We will build a digital clock to demonstrate the fundamentals of React and Node apps.
...
The clock in our UI should now automatically update every second!&#x20;

TODO: Insert reference solution

{% hint style="info" %}
**Reference solution at bottom of page**
...
We now have a clock that we can re-use in anywhere in our app with a single line of code!&#x20;
...
Great job on making a clock app that shows multiple time zones! Don't forget to review the reference solution at the bottom of the page to see how Rocket implemented ours. Coding is like writing an essay and there are many right answers, so don't fret if yours looks different.

## Comfortable: Improve clock UI with React Bootstrap grid system
