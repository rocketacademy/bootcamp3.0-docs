...
When you ran `flyctl launch` a Dockerfile was generated, we will need to ensure that our  `release.sh` file is used an ran within the fly.io envrioment. Alter the Docker file to include the line below, ensure that it is done following RUN npm install:
