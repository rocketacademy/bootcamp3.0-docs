...
Most of `db/models/user.cjs` where we define our `User` model is boilerplate except for `this.hasMany(models.Post)` where we declare `User`'s association with the `Post` model.
...
The migration to create the `Posts` table contains the `AuthorId` foreign key, declared almost identically as it was in `db/models/post.cjs` above. Documentation for declaring foreign keys in migrations [here](https://sequelize.org/api/v6/class/src/dialects/abstract/query-interface.js\~queryinterface#instance-method-createTable).
