...
In [2.3.3: POST Requests](../3.3-express-js/3.3.3-handling-post-requests.md#receive-post-request-data-as-keys-and-values), we added an `express.urlencoded` middleware to populate `request.body`. For file uploads, we will add a similar configuration to Express using the [Multer](https://github.com/expressjs/multer) library to accept binary file data in requests.
