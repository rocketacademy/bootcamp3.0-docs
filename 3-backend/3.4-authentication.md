...
2. To retrieve this access token on our frontends, we need to pass additional `audience` and `scope` props to the `Auth0Provider` higher-order component.
   1. `audience` is the domain of our API server. This may be stored in an environment variable, assuming our domain will differ depending on the environment.
...
Some routes in our backend will be protected, for example routes to access user data. Routes that expose user data should only expose user data that the logged-in user is authorised to see.
