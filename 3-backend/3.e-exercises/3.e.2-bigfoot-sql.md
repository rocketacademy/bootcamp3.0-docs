...
2. Create a route for a POST request to `/` to create a new sighting in our database using Sequelize and return the new sighting. We can access request body attributes using `req.body`.
...
2. Create new component for new sighting form in the `components` folder and import it in `index.js` where we can render it for the `/new` frontend route. This component should render a sighting form that sends a POST request to our API server on submit and redirects to the `/` route after submit, where the `/` route should display the new sighting.
   ...
   5. Consider using [React Router's `useNavigate` hook and `navigate` function](https://reactrouter.com/docs/en/v6/getting-started/overview#navigation) to navigate to the home page `/` after submitting the form and sending the form data to our API server. Navigate after the `axios` promise resolves so that when we reach the home page `/` our app will be able to retrieve the new sighting from the database.
