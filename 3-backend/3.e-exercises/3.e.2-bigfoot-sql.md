...
![Sample view of Sightings table in Postico after running migrations and seeders](<../../.gitbook/assets/image (1).png>)
...
![Sample output of a request to localhost:3000 from Thunder Client](../../.gitbook/assets/image.png)
...
3. Tweak the logic in the frontend to show only the `date` property of each sighting on the homepage and `date`, `location` and `notes` properties of each sighting on sighting-specific pages
...
3.  Before writing other request handler logic, verify our route is receiving the request correctly with `console.log` like the following.
    ...
    ```
4. Use Thunder Client to confirm our server receives requests correctly.
...
#### Add middleware to read POST request bodies
...
#### Test new route with Thunder Client

TODO: Include instructions to test POST route with Thunder Client
