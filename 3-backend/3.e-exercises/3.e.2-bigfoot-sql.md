...
```
npx sequelize model:generate --name Sighting --attributes date:date,location:string,notes:text
```
...
#### Update model and migration file extensions to `.cjs`

Notice the contents of Sequelize-generated model and migration files use `module.exports` instead of `export default` syntax. Because of this we will need to update our model and migration file extensions from `.js` to `.cjs` for Node to execute these file as "CommonJS" modules that do not use `import` and `export` syntax.

We will also need to update `db/models/index.cjs` to read files with the `.cjs` file extension on lines 25-27. Replace lines 25-27 with the following. Notice we changed the final boolean expression to `file.slice(-4) === ".cjs"`.

{% code title="db/models/index.cjs" %}
```javascript
return (
...
#### Update seed file extension to `.cjs`

Similar to what we did with model and migration files above, update our seed file extenion to `.cjs` so Node can read it.
...
2. Update `config/database.json` to use your Unix username and name the database `bigfoot_sql_development`. Kai's looks like the one below.
3. Run `npx sequelize db:create` to create the `bigfoot_app_development` database
4. Run `npx sequelize db:migrate` to set up the database schema
5. Run `npx sequelize db:seed:all` to seed data in the database
6. Verify the seed data was added by viewing the `Sightings` table in your chosen SQL client. You can use the information in `config/database.json` to connect to the database in the SQL client.
...
2. Open Thunder Client in VS Code and send a request to `localhost:3000`
3. Verify that we receive the 3 sightings we seeded in our database

![Sample output of a request to localhost:3000 from Thunder Client](<../../.gitbook/assets/3.E.2 - Bigfoot SQL - Thunder Client>)

#### Verify sighting-specific route returns data for single sighting

1. Start the API server with `npm start` from the root of the Bigfoot SQL Backend repo
2. Send a request to `localhost:3000/1` in Thunder Client
...
2. Create a route for a POST request to `/` to create a new sighting in our database using Sequelize and return the new sighting with `res.json`. We can access request body attributes using `req.body`.
   1. Note we design our API to send a POST request to `/` instead of paths like `/sighting` or `/createSighting` because our app only has the sighting feature at the moment and [REST API best practices](https://stackoverflow.blog/2020/03/02/best-practices-for-rest-api-design/) suggest using nouns instead of verbs in paths, using request method to communicate type of action
3. Verify our route works by sending a POST request with Thunder Client to `localhost:3000` with the relevant new sighting data in a JSON body

Your Thunder Client output may look something like the following.

![POST request with Thunder Client to our API server at localhost:3000](<../../.gitbook/assets/3.E.2 - Bigfoot SQL - Thunder Client POST.png>)
