...
1. Add `audience` and `scope` props to `Auth0Provider` HOC in `index.js` in our frontend. Auth0 provides us the relevant values in the above docs after we log in. Reminder to select the relevant Auth0 application in in the menu above the code snippet in the docs.
2. TODO: Reword following. Retrieve `getAccessTokenSilently` from `useAuth0` hook in sell and buy contexts
3. Use `getAccessTokenSilently` to get access token
4. Pass access token to `axios` in header
