...
Here is reference code for the [frontend](https://github.com/rocketacademy/bigfoot-frontend-bootcamp/tree/solution-sql-m-m-base/src/components) and the [backend](https://github.com/rocketacademy/bigfoot-sql-backend-bootcamp/tree/solution-sql-m-m-base) for this exercise, and here is a reference deployment. You can do better!
...
2. We can skip the "Declare app dependencies" section because our Bigfoot repo is already an NPM project with `package.json`.
3. Specify our Node version in `package.json` as per Heroku's instructions. Our node version should be "16.x"; you can check yours by typing `node --version` in your command line.
4. Create a file called `Procfile` in the root of our repo with the contents `web: node index.js` on the first line. This will tell Heroku to run that command to start our app.
5. Update the line of code that declares `PORT` near the top of `index.js` to first try to read `PORT` from an environment variable. Rocket's reference code looks like this: `const PORT = process.env.PORT || 3000;`. This will allow our app to read from Heroku's `PORT` environment variable in Heroku but fall back to port `3000` locally.
6. Create a local file `.env` in the root of the repo with contents `PORT=3000` so that Heroku always starts our app locally with port 3000. Add `.env` to its own line in `.gitignore` so that local environment variables are not committed to GitHub.
7. Test Heroku's app execution locally with `heroku local web`. Fix any issues here before proceeding to deployment.
8. Copy Heroku's suggested `.gitignore` contents into our `.gitignore` file to avoid saving any build artifacts (files generated from the package-building process) to GitHub.
9. Add a `production` Sequelize config to our `config/database.json` file below our `development` config. Rocket's config is copied below. This is what our app needs to access Heroku Postgres.
...
10. Follow instructions in "Deploy your application to Heroku" section to deploy. If your latest code is not on the `main` branch, run `git push heroku <your_branch_name>:main` to deploy your feature branch to Heroku's `main` branch.
11. Provision our Postgres database in Heroku by logging into Heroku, navigating to the [Heroku Postgres add-on page](https://elements.heroku.com/addons/heroku-postgresql) and clicking "Install Heroku Postgres" in the top right corner. When prompted, select the "Hobby Dev - Free" plan and the Heroku app we just created and click "Submit Order Form". This will provision a free Heroku Postgres database instance for our backend.
12. Execute migrations and seeders remotely on our Heroku app. We use `npx sequelize-cli` instead of `npx sequelize` because `sequelize-cli` is a dev dependency and removed from Heroku after the app is built.
    1. `heroku run npx sequelize-cli db:migrate`
    2. `heroku run npx sequelize-cli db:seed:all`
13. Test our backend with Thunder Client! We should now be able to query our API server and retrieve results from our seed data.
