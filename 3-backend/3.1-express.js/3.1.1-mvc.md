...
In order to test out the POST request you need to alter the request within ThunderClient to send a POST request not a GET request. We can do this at the top of the window, with this in mind as the POST request is mocking a form submission we will need to attach the form data you can use a JSON object to acheive this. Rememer that the data you add will interface with your database and thus the data's keys need to match column names.&#x20;
