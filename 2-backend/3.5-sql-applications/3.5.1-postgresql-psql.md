...
Part of setting up database servers is setting up database schema, i.e. what tables and columns are in the database. We'll learn more about how to design database schema in [3.4.6: SQL Schema Design](../3.4-sql-language/3.4.6-sql-schema-design.md), but for now Rocket will provide database schema that we will implement. Web applications typically do not manage database schema; application code depends on database schema, but the schema is not defined by the application.
