...
2. Use EJS partials \(see [2.4.3: EJS Partials](../2.4-ejs/2.4.3-ejs-partials.md)\) to re-use navbar code from a single EJS template.
...
Add a visit counter to the home page that keeps count of all home page visits \(including repeat visits\) via cookies. See [2.6.1: Cookies](../2.6-authentication/2.6.1-cookies.md) for a refresher.
