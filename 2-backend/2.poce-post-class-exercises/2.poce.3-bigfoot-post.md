...
2. Remember to include the [middleware for Express to populate `request.body`](../2.3-express-js/2.3.3-handling-post-requests.md#receive-post-requests-in-express).
...
1.  Create a form on the sightings list page that sends a GET request with the same sort query parameters from [2.POCE.1: Bigfoot](2.poce.1-bigfoot.md#sort-by). The generated request URLs might look like the following.
...
3.  Add a similar dropdown for other sort options from [2.POCE.2: Bigfoot EJS](2.poce.2-bigfoot-ejs.md#sort-by). These might produce the following URLs.
